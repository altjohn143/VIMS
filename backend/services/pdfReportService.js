...
class PDFReportService {
  ...
  _addReportHeader(doc, title, period, date, month, creator, generatedAt = new Date(), timezoneOffsetMinutes = 0) {
    ...
    doc.fontSize(20).font(this.fonts.bold).text('VIMS - Village Integrated Management System', 0, 50, this._getTextOptions({ align: 'center' }));

    // Title
    doc.moveDown(2);
    doc.fontSize(16).font(this.fonts.bold).text(title, this._getTextOptions({ align: 'center' }));

    // Period/Date info
    doc.moveDown(1);
    doc.fontSize(12).font(this.fonts.normal);
    if (period && date) {
      doc.text(`${period}: ${date}${month ? `/${month}` : ''}`, this._getTextOptions({ align: 'center' }));
    }
    doc.text(`Generated on: ${displayTime.toLocaleString()}`, this._getTextOptions({ align: 'center' }));
    ...
    if (creator) {
      ...
      doc.fontSize(10).font(this.fonts.normal).text(
        ...
        this._getTextOptions({ align: 'center' })
      );
    }
