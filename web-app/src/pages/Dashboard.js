...
const Dashboard = () => {
  ...
  const roleConfig = {
    resident: {
      ...
      features: {
        visitors: [
          { title: 'Generate Visitor Pass', icon: <QrCodeIcon />, link: '/visitors' },
        ],
        services: [
          { title: 'Service Requests', icon: <BuildIcon />, link: '/service-requests' },
        ],
        payments: [
          { title: 'Pay Dues', icon: <PaymentIcon />, link: '/payments' }
        ],
        community: [
          { title: 'Announcements', icon: <AnnouncementIcon />, link: '/announcements' }
        ],
        settings: [
    ...
    admin: {
      ...
      features: {
        visitors: [
          { title: 'Visitor Management', icon: <PeopleIcon />, link: '/admin/visitor-management' },
        ],
        users: [
    ...
    security: {
      ...
      features: {
        scanner: [
          { title: 'QR Code Scanner', icon: <ScannerIcon />, link: '/security/scanner' }
        ],
        visitors: [
        ...
        patrol: [
          { title: 'Patrol Schedule', icon: <AssignmentIcon />, link: '/security/schedule' }
        ],
        services: [
          { title: 'Service Requests', icon: <BuildIcon />, link: '/security/service-requests' }
        ],
        incidents: [
          { title: 'Incident Reports', icon: <AssignmentIcon />, link: '/security/incidents' }
        ],
        settings: [
          { title: 'Profile Settings', icon: <SettingsIcon />, link: '/profile' }
        ]
  ...
  const accountInfo = [
    ...
    { 
      label: 'Role', 
      value: <Chip label={user.role} size="small" sx={{ 
  ...
  const miniSidebar = (
    <Box sx={{ 
      ...
      {/* Role-specific icons (first 3 important actions) */}
  ...
  const fullSidebar = (
    <Box sx={{ width: 280, height: '100%', bgcolor: '#1e293b', color: 'white' }}>
      {/* Sidebar Header */}
      <Box sx={{ p: 3, textAlign: 'center', borderBottom: '1px solid #374151' }}>
        ...
        />
        {user.houseNumber && (
          <Typography variant="caption" sx={{ mt: 1, color: '#94a3b8', display: 'block' }}>
  ...
  return (
    <>
      ...
      <AppBar position="fixed" sx={{ 
        ...
        <Toolbar>
          {/* Burger Menu Button (Shows different icons based on sidebar state) */}
          <IconButton
            edge="start"
            color="inherit"
            aria-label="menu"
        ...
        <Container maxWidth="lg" sx={{ py: 4 }}>
          ...
          <Paper sx={{ 
            ...
            '&:hover': {
              ...
              boxShadow: '0 12px 40px rgba(34, 36, 190, 0.4)'
            }
          }}>
          ...
          <Paper sx={{ 
            ...
            <Typography variant="body1" sx={{ 
              ...
              {user.role === 'resident' && 
                'Manage your visitor passes, service requests, and community payments all in one place. Use the menu on the left to access all features.'}
              {user.role === 'admin' && 
                'Monitor system activities, manage users, and oversee all community operations. Expand the sidebar to access all admin tools.'}
              {user.role === 'security' && 
                'Handle visitor approvals, monitor security logs, and manage on-site activities. Click the menu icon to access security tools.'}
            </Typography>
            {!sidebarOpen && (
              <Button 
                variant="outlined" 
